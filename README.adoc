...
The project **connector-merge-pdf** is a Bonita Connector for **Bonita {short-bonita-version}**  **Community** version written in `java` language.
...
[source,bash]
----
git clone https://your.github.com/connector-merge-pdf.git
cd connector-merge-pdf/
./mwnw package
----

The build should produce a jar archive under the `target/` folder named `connector-merge-pdf-1.0-SNAPSHOT.jar`
